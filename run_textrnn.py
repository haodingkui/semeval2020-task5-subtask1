""" Train the TextRNN model for sentence classification. """
...
from models.modeling_textrnn import RNN
from configs.configuration_textrnn import TextRNNConfig
...
def evaluate(model, iterator, criterion, eval_mode):
    ...
    if eval_mode == "test":
        ...
        output_pred_file = os.path.join("output/SemEval-2020-Task5/textcnn/", "submission.csv")
...
def main(args):
    ...
    for n_fold in range(len(iterators)):
        ...
        for epoch in range(N_EPOCHS):
            ...
            print(f'Fold:{n_fold+1:02} | Epoch: {epoch+1:02} | Epoch Time: {epoch_mins}m {epoch_secs}s')
            print(f'\tTrain Loss: {train_loss:.3f}')
            print(f'\t Val. Loss: {valid_loss:.3f} |  Val. Acc: {valid_acc*100:.2f}% | Val. Precision: {valid_precision*100:.2f}% | Val. Recall: {valid_recall*100:.2f}% | Val. F1: {valid_f1*100:.2f}%')
    ...
    model.load_state_dict(torch.load('output/SemEval-2020-Task5/textcnn/model.pt'))
